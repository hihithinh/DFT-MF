    ...
    <div class="bg-surface-container rounded-xl shadow-2xl max-w-6xl max-h-[90vh] w-full mx-4 overflow-hidden" @click.stop>
      ...
      <div class="bg-surface-container-high p-6 border-b border-outline-variant/20">
        <div class="flex items-center justify-between">
          <div>
            <h3 class="text-2xl font-headline font-bold text-on-surface">Analyzed Mouth Frames</h3>
            <p class="text-sm text-secondary mt-1">Deepfake analysis results (REAL/FAKE)</p>
          </div>
          <button 
            @click="closeModal"
      ...
      <div class="bg-surface-container-high p-4 border-t border-outline-variant/20">
        <div class="flex justify-between items-center">
          <p class="text-sm text-secondary">
            {{ analyzed.length }} analyzed samples
          </p>
...
export default {
  ...
  emits: ['close', 'retry-step-3'],
  methods: {
    closeModal() {
      this.$emit('close')
    },
    retryStep() {
      this.$emit('retry-step-3')
    }
  },
  watch: {
    show(newVal) {
